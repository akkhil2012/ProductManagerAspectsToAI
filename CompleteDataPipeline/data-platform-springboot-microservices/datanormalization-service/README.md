...
This is a Spring Boot microservice for datanormalization operations in the data platform.
...
- `POST /api/v1/datanormalization` - Create a new record
- `GET /api/v1/datanormalization` - Get all records (with optional filtering)
- `GET /api/v1/datanormalization/{id}` - Get record by ID
- `GET /api/v1/datanormalization/record/{recordId}` - Get record by record ID
- `PATCH /api/v1/datanormalization/{id}` - Update a record
- `DELETE /api/v1/datanormalization/{id}` - Delete a record

### Python Integration Endpoints
- `POST /api/v1/datanormalization/process` - Process record via Python FastAPI
- `POST /api/v1/datanormalization/validate` - Validate record via Python FastAPI

### Utility Endpoints
- `GET /api/v1/datanormalization/count?status={status}` - Get record count by status
...
### Running with Docker
```bash
docker build -t datanormalization-service .
docker run -p 8084:8084 datanormalization-service
```
...
Connection details:
- JDBC URL: `jdbc:h2:mem:datanormalizationdb`
- Username: `sa`
- Password: `password`
...
### Create a Record
```bash
curl -X POST http://localhost:8084/api/v1/datanormalization \
...
### Get All Records
```bash
curl http://localhost:8084/api/v1/datanormalization
```

### Update a Record
```bash
curl -X PATCH http://localhost:8084/api/v1/datanormalization/1 \
  -H "Content-Type: application/json" \
  -d '{
    "status": "COMPLETED",
...
### Process via Python
```bash
curl -X POST http://localhost:8084/api/v1/datanormalization/process \
  -H "Content-Type: application/json" \
  -d '{
    "recordId": "REC-001",
